...
public class CartServiceImple implements ICartService {
	...
	public void editCart(Cart c) {
		...
		Product p = productDao.findById(cart.getProducts().getId()).orElseThrow(() -> new RuntimeException("Invalid UserId"));
		if(c.getQuantity()<=p.getQty())
		{
	...
	public void addToOrder(Order o, int id)
	{
		...
		for(Cart cart: c)
		{
			Product pro = productDao.findById(cart.getProducts().getId()).orElseThrow(()->new RuntimeException("Product Not Found"));
			pro.setQty(pro.getQty()-cart.getQuantity());
			list.add(pro);
		}
